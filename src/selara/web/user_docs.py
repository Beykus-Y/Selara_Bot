...
_USER_DOC_SECTIONS: tuple[dict[str, Any], ...] = (
    ...
    _docs_section(
        ...
        _docs_item(
            ...
            text=(
                "Сначала посмотрите свой статус через `/relation`, затем отправьте предложение пары или брака. "
                "Цель выбирается через reply или `@username`, а подтверждение идёт кнопками."
            ),
            badges=("группа", "reply"),
            commands=("/relation", "/pair @username", "/marry @username"),
            triggers=("отношения", "брак", "пара", "жениться"),
            examples=("reply + /pair", "reply + /marry", "/pair @username", "/marry @username"),
            notes=(
                "Срок ответа на предложение: 24 часа.",
                "В `/relation` бот показывает текущий статус, партнёра и готовность действий.",
            ),
        ),
        _docs_item(
    ...
    _docs_section(
        ...
        _docs_item(
            ...
            examples=("reply + обнять", "reply + дать пять", "reply + подмигнуть"),
        ),
        _docs_item(
            "18+ reply-действия",
            text=(
            ...
            triggers=("трахнуть", "соблазнить", "засосать", "провести ночь с"),
            examples=("reply + трахнуть", "reply + соблазнить", "reply + провести ночь с"),
            notes=(
