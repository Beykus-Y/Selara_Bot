...
class FakeRepo:
    ...
    async def upsert_activity(self, *, chat: ChatSnapshot, user: UserSnapshot, event_at: datetime) -> ActivityStats:
        self.calls.append((chat, user, event_at))
...
async def test_track_activity_use_case_builds_snapshots() -> None:
    ...
    chat, user, event_at = repo.calls[0]
