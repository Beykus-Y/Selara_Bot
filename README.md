...
- Поддерживаемые режимы включают: `spy`, `mafia`, `dice`, `quiz`, `bredovukha`, `bunker`, `whoami`, `zlobcards`.
...
#### 2.6 AI-ассистент и голос (опционально, требует отдельного включения)
...
- `GACHA_ADMIN_USER_ID`, `GACHA_ADMIN_TOKEN`
- `WEB_AUTH_SECRET`
...
- **[CONTRADICTIONS.md](CONTRADICTIONS.md)**, **[FUNCTIONAL_GAPS_ANALYSIS.md](FUNCTIONAL_GAPS_ANALYSIS.md)**,
  **[IDEAS.md](IDEAS.md)**, **[ACTIONS.md](ACTIONS.md)** — архивные снимки прошлых
  разборов, не живые документы (см. пометку в начале каждого файла).
- **[docs/bot_docs/](docs/bot_docs/)** — техническая документация, которую читает
  сам AI-ассистент бота через `read_bot_doc`/`list_bot_docs` (роли, модерация,
  ресты, образы, словарь, возможности/ограничения) — см. раздел 2.6.
