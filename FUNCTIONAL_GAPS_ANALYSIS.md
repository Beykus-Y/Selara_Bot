...
> **Архивный документ.** Проверено 2026-08-20 (аудит документации): пункт
> №1 (chat_write_locked не блокирует команды) не подтверждается кодом —
> `src/selara/presentation/middlewares/chat_write_lock.py` уже реализует
> именно эту блокировку. Документ также противоречит сам себе по пункту
> «`когда был @username`» (№3 говорит «работает», №8 говорит «не работает»
> для одного и того же поведения) — актуальное состояние: работает
> (см. `BUGS.md` №3). Остальные пункты не перепроверялись в рамках этого
> аудита. Файл оставлен как снимок анализа на апрель 2026, не редактируется
> как живой список — актуальный источник истины: код на `main` и `BUGS.md`.
