...
## GUI редактор баннеров

Для локальной настройки JSON есть отдельный инструмент:

```bash
cd gacha
python3 tools/banner_editor.py
```

Что умеет сейчас:

- открыть и сохранить banner JSON;
- редактировать поля баннера и карточек;
- задавать `region_code` и `element_code`;
- сразу видеть шанс конкретной карты, шанс по редкости и сумму весов;
- быстро добавлять, дублировать, удалять и сортировать карты.
...
- `rarity` — редкость.
  Допустимые значения сейчас: `common`, `rare`, `epic`, `legendary`, `mythic`.
...
- `region_code` — код региона персонажа.
  Для `genshin` поле обязательно, для `hsr` и других баннеров можно не указывать.
  Например: `mondstadt`, `liyue`, `inazuma`, `sumeru`, `fontaine`, `natlan`, `nod_krai`, `snezhnaya`.
- `element_code` — код стихии/элемента персонажа.
  Для `genshin` поле обязательно. Рекомендуемые коды: `hydro`, `electro`, `pyro`, `cryo`, `anemo`, `dendro`, `geo`.
